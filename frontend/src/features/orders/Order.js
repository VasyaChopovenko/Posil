...
export default function Order({id}) {
    ...
    const orderItems = order.items.map(item => <OrderItem key={item.id} productId={item.product_id} itemCount={item.count}/>);

    return (
        <Accordion.Item eventKey={order.id}>
            <Accordion.Header className="m-0">
                <div className="d-flex justify-content-between">
                    <p style={{minWidth: '10rem', maxWidth: '10rem'}} className="mb-0 me-5">{getCreationDate()}</p>
                    <p style={{minWidth: '7rem', maxWidth: '7rem'}} className="mb-0 me-5">{order.status}</p>
                    <p style={{minWidth: '28rem', maxWidth: '28rem'}} className="mb-0 me-5">{order.clientAddress}</p>
